        	...
        	<h6 class="heading font-x1"><a href="#">Livre</a></h6>
         	<?php
				...
				$db_found = mysqli_select_db($db_handle, $database);
					...
					if ($db_found) 
					{
						$max = "SELECT MAX(Nbvendu) FROM livre";
						//$sql = "SELECT Titre, Auteur, Prix, Image, Nb_vendu FROM livre WHERE Nb_vendu='$max'";
						//$sql = "SELECT Titre, Auteur, Prix, Nbvendu FROM livre WHERE Nbvendu LIKE MAX(Nbvendu)";
						$test = mysqli_query($db_handle, $max);
						$sql="SELECT Titre FROM livre WHERE Nbvendu LIKE "$test" ";
						$result = mysqli_query($db_handle, $sql);
						$donnees = mysqli_num_rows($result);
						echo $donnees['Titre'];
						/*$test = mysqli_query($db_handle, $max);
						$test1 = mysqli_fetch_array($test);
						echo $test1['MAX(Nbvendu)'];
						Warning: mysqli_fetch_array() expects parameter 1 to be mysqli_result, boolean given in C:\wamp64\www\v8\index.php on line 152
Call Stack*/
						//echo "'$data'";
        ...
        <article><a href="#"></a>
          ...
          <?php
				...
				$db_found = mysqli_select_db($db_handle, $database);
					...
					if ($db_found) 
					{
						//$max = "SELECT MAX(Nb_vendu) FROM musique";
						//$sql = "SELECT Titre, Auteur, Prix, Image, Nb_vendu FROM livre WHERE Nb_vendu='$max'";
						//$sql = "SELECT Titre, Auteur, Prix, Nbvendu FROM livre WHERE Nbvendu LIKE MAX(Nbvendu)";
						//$test = mysqli_query($db_handle, $max);
						$sql= "SELECT Titre FROM musique WHERE Nb_vendu LIKE MAX(Nb_vendu)";
						$result = mysqli_query($db_handle, $sql);
          ...
          <?php
				...
				$db_found = mysqli_select_db($db_handle, $database);
					...
					if ($db_found) 
					{
						...
						//$sql = "SELECT Titre, Auteur, Prix, Nbvendu FROM livre WHERE Nbvendu LIKE MAX(Nbvendu)";
						//$test = mysqli_query($db_handle, $max);
						$sql= "SELECT Nom FROM sport WHERE Nb_vendu LIKE MAX(Nb_vendu)";
						$result = mysqli_query($db_handle, $sql);
						$donnees = mysqli_fetch_array($result);
						echo $donnees['Nom'];
						/*$test = mysqli_query($db_handle, $max);
        ...
        <article><a href="#"></a>
          ...
          <?php
				...
				$db_found = mysqli_select_db($db_handle, $database);
					...
					if ($db_found) 
					{
						$max = "SELECT MAX(Nb_vendu) FROM vetement";
						//$sql = "SELECT Titre, Auteur, Prix, Image, Nb_vendu FROM livre WHERE Nb_vendu='$max'";
						//$sql = "SELECT Titre, Auteur, Prix, Nbvendu FROM livre WHERE Nbvendu LIKE MAX(Nbvendu)";
						$test = mysqli_query($db_handle, $max);
						$sql= "SELECT Nom FROM vetement WHERE Nb_vendu=$test";
						$result = mysqli_query($db_handle, $sql);
						$donnees = mysqli_fetch_array($result);
						echo $donnees['Nom'];
						/*$test = mysqli_query($db_handle, $max);
